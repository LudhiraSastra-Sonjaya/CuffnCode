...
dashboard_gui.py
----------------
Main SCADA HMI Dashboard for the Smart Parking Multi-MCU Distribution
Pipeline Workflow simulation.

This module owns all Tkinter widget construction, the 6-stage pipeline
orchestration via non-blocking `root.after()` callbacks, real-time
Matplotlib graph rendering, and the communication log console.

Dependencies (nodes & simulator are imported from their own modules):
    - nodes.WorkerMCU   : virtual edge Worker MCU
    - nodes.MasterMCU   : virtual central Master MCU
"""
...
from nodes.worker_mcu import WorkerMCU
from nodes.master_mcu import MasterMCU
...
class SmartParkingDistributionWorkflowGUI:
    ...
    def set_workflow_stage(self, active_stage: str, status_text: str):
        """
